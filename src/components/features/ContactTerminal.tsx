...
export default function ContactTerminal() {
    ...
    return (
        <section className="w-full max-w-4xl mx-auto px-6 py-32 relative">
            ...
            {/* Lueur d'arrière-plan statique (donne de la profondeur sans bouger) */}
            ...
            <motion.div
                // Animation simple et propre : Montée + Opacité
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
                transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} // Ease premium
            ...
            >
                ...
                <form onSubmit={handleSubmit((d) => console.log(d))} className="p-8 md:p-12 space-y-12">
                    ...
                    {/* Bouton : Minimaliste & Impactant */}
                    <motion.button
                        whileHover={{ scale: 1.005, backgroundColor: "rgba(255,255,255,1)", color: "#000" }}
                        whileTap={{ scale: 0.99 }}
                        className="w-full bg-studio-neon text-black font-black py-5 rounded-xl uppercase tracking-[0.2em] text-[11px] transition-colors duration-300 flex items-center justify-center gap-3"
                    >
                        <Send size={14} />
